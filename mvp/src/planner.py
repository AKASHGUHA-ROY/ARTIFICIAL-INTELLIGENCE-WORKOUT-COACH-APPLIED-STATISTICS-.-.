...
def add_exercise(
    ...
    """Track minutes and rounds without printing the same exercise many times."""
    name = exercise["name"]
    ...
    bucketed = {
        bucket: sort_exercises(
            [
                ...
                if get_full_body_bucket(exercise) == bucket
            ],
...
def build_workout_plan(
    ...
    if focus == "full_body":
        rationale_bits.append(
            "started with upper body, lower body, and core/cardio coverage"
        )

        if minutes_available <= 15:
            rationale_bits.append(
                "leaned on more workout-style moves for the short session"
            )
    else:
        rationale_bits.append(
