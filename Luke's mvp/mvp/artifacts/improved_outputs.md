...
**Notes:** Better than the baseline because the workout now starts with a more demanding main exercise instead of opening with warmup-like movements. Remaining weakness: the no-equipment arm exercise pool is still small, so variety is limited.
...
**Notes:** Better than the baseline because larger rowing movements are clearly prioritized before support work. Remaining weakness: the planner still does not assign sets, reps, or rest periods, so the output is closer to exercise selection than a full coaching plan.
...
**Notes:** Better than the baseline because the plan stays conservative and excludes knee-aggravating options while still giving the user a usable workout. Remaining weakness: the pain handling is broad and cannot personalize for severity, diagnosis, or rehab needs.
...
**Notes:** Better than the baseline because the short session feels more energetic and practical, with clear upper-body, lower-body, and conditioning coverage. Remaining weakness: the planner still uses simple bucket rules and cannot yet build richer short circuits or supersets.
