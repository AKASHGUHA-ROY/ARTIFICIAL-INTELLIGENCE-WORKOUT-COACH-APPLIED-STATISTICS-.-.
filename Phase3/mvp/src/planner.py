...
def sort_exercises(exercises: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    """Prefer main moves first, then support, then warmups."""
    return sorted(
        exercises,
        key=lambda item: (
...
def build_targeted_plan(
    ...
    # First pass: take unique exercises in quality order.
...
def build_workout_plan(
    ...
    if focus == "full_body":
        ...
        rationale_bits.append(f"preferred main {focus.replace('_', ' ')} exercises first")
